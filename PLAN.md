...
| 버튼 크기 | **중요도 세 단계** — `lg` 52px(그 화면의 할 일) · `md` 44px(기본) · `sm` 36px(곁가지·잔 동작). 높이는 `SIZE_CLASS` 에서만 정한다 | 두 단계 시절엔 둘 다 44px 이라 글자만 달랐다. "24가지 강점 보기" 같은 곁길이 해야 할 일과 같은 덩치로 앉아 있었다 |
| 눌러야 할 것의 크기 | 명단의 이름·강점 칸은 버튼이 아니라 **56px 칸**. 어떤 버튼보다 크다 | 이 앱에서 실제로 누르는 것은 버튼이 아니라 이름과 강점이다 |
